...
func TestOutOfGasInCacheFlush(t *testing.T) {
	...
	for i := 0; i < 11; i++ {
		args = multicallAppendStore(args, testhelpers.RandomHash(), testhelpers.RandomHash(), false, false)
	}
	args = multicallAppendStore(args, testhelpers.RandomHash(), testhelpers.RandomHash(), false, true)

	// fails during storage flush
	tx = builder.L2Info.PrepareTxTo("Owner", &multiAddr, uint64(310000), nil, args)
	err = builder.L2.Client.SendTransaction(ctx, tx)
	Require(t, err)
	receipt, err = EnsureTxSucceeded(ctx, builder.L2.Client, tx)
